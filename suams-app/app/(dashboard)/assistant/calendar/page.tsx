...
export default function AssistantCalendarPage() {
  ...
  useEffect(() => {
    // Set up mock calendar events matching status colors
    const mockEvents = [
      { id: 'e1', title: 'Zahid Mahmood - CS BS', date: 14, time: '10:00 - 10:20', status: 'Pending', desc: 'Degree signature verification request' },
      { id: 'e2', title: 'Dr. Asif - Teacher', date: 14, time: '11:00 - 11:30', status: 'Approved', desc: 'HEC grant approval discussion' },
      { id: 'e3', title: 'Saira Bibi - Parent', date: 15, time: '09:30 - 09:50', status: 'Pending', desc: 'Late fee deadline discussion request' },
      { id: 'e4', title: 'Kamran Shah - Alumni', date: 15, time: '12:00 - 12:20', status: 'Rejected', desc: 'Transcript verification appeal' },
    ];
  ...
  const startOffset = 3; // Mocking calendar offsets
    ...
    <div className="max-w-6xl mx-auto py-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
            Visualize schedule load and manage timings interactively.
          </p>
        </div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <button onClick={handlePrevDate} className="p-2 rounded-xl border border-white/5 bg-black/20 text-white">
            <ChevronLeft size={16} />
          </button>
          <span className="text-base font-bold text-white">July 2026</span>
          <button onClick={handleNextDate} className="p-2 rounded-xl border border-white/5 bg-black/20 text-white">
            <ChevronRight size={16} />
          </button>
        </div>
        <span className="text-xs font-mono" style={{ color: 'var(--text-secondary)' }}>SECRETARIAT PORTAL CALENDAR</span>
      </div>
      ...
      {viewMode !== 'Month' && (
        <div className="glass-card rounded-2xl p-16 border border-white/5 text-center bg-black/25">
          ...
          <p className="text-xs max-w-sm mx-auto" style={{ color: 'var(--text-secondary)' }}>Switch to Month view to manage current upcoming bookings queue.</p>
        </div>
      )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-sm glass-card rounded-2xl p-6 border border-[var(--border)]">
              <div className="flex justify-between items-start mb-4 pb-2 border-b border-white/5">
                <div>
                  <span className="text-[10px] font-mono text-amber-400 font-bold">CALENDAR EVENT</span>
              ...
              <div className="flex gap-3 justify-end">
                <button onClick={() => setModalOpen(false)} className="px-4 py-2 rounded-xl text-xs font-semibold bg-zinc-800 text-white w-full">Close popup</button>
              </div>
            </motion.div>
          </div>
        )}
