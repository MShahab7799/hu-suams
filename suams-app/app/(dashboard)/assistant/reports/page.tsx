  ...
  return (
    <div className="max-w-5xl mx-auto py-6">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
            Generate analytical reviews and export logs for files validation audits.
          </p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-8">
        {/* Statistics Cards */}
        {[
          { label: 'Weekly Completion Rate', value: '88.4%', change: '↑ 2.3% vs last week', icon: CheckCircle2, color: '#22C55E' },
          { label: 'Peak Hour Load', value: '11:00 AM', change: '5 appointments avg', icon: BarChart3, color: '#C5A028' },
          { label: 'Peak Department', value: 'Computer Science', change: '42% of total requests', icon: Users, color: '#3B82F6' },
        ].map((stat) => {
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    ...
                    <option>Department-wise Load</option>
                  </select>
                </div>
                <div>
                  <label className="block font-bold uppercase mb-2 text-zinc-400">Include Audit Logs</label>
                  <select className="w-full p-3 rounded-xl border border-white/5 bg-black/35 text-white">
                    <option>Yes, include changes timeline</option>
                    <option>No, stats tables only</option>
                  </select>
                </div>
              </div>
        ...
        <div className="glass-card rounded-2xl p-5 border border-white/5 flex flex-col justify-between">
          <div>
            <h3 className="font-bold text-xs uppercase tracking-widest text-white mb-4 pb-2 border-b border-white/5 flex items-center gap-1.5">
              <PieChart size={14} className="text-amber-400" /> peak load hours distribution
            </h3>
            <div className="flex flex-col gap-3 text-xs" style={{ color: 'var(--text-secondary)' }}>
              <div className="flex justify-between">
                <span>09:00 – 10:00</span>
                <span className="font-mono text-white">15% load</span>
              </div>
              <div className="flex justify-between">
                <span>10:00 – 11:00</span>
                <span className="font-mono text-white">35% load</span>
              </div>
              <div className="flex justify-between">
                <span>11:00 – 12:00</span>
                <span className="font-mono text-amber-400 font-bold">45% load (Peak)</span>
              </div>
              <div className="flex justify-between">
